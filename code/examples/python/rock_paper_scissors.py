...
def generate_play_string():
    string = ""
    for num in range(len(options)):
        string += options[num].capitalize()
        if num != len(options):
            string += ", "
    return string + ": "
...
def main():
    ...
    while True:
        ...
        if player == computer:
            ...
            print("Computer wins! %s beats %s" % (options[computer], options[player]))
        else:
            print("Player wins! %s beats %s" % (options[player], options[computer]))
