...
: x{_x}, y{_y}, w{_w}, h{_h}, camera{0, 0, 10}, color{} {
...
void TileField::end_event() {
  ...
  SDL_Log("start fill from %d", to_fill);
  Color replace = real[to_fill];
  SDL_Log("replacing %d,%d,%d tiles with %d,%d,%d", replace.r, replace.g, replace.b, color.r, color.g, color.b);
  std::set<int> pixels;
  pixels.insert(to_fill);
  while (pixels.size() != 0) {
    SDL_Log("round n, size is %lu", pixels.size());
    ...
    */
    ...
    for (auto i : getneighbors(*it, 100, 10000)) {
      // SDL_Log("check %d neighbor %d", *it, i);
      if (real[i] == replace) {
        // SDL_Log("add neighbor %d", i);
        pixels.insert(i);
      } else {
        SDL_Log("not adding %d, it's %x,%x,%d", i, real[i].r, real[i].g, real[i].b);
        // real[i] = {0xff, 0, 0};
      }
    }
    pixels.erase(it);
...
Palette::Palette(int _x, int _y) : x{_x}, y{_y} { }
...
void Palette::handle_click(int click_x, int click_y) {
...
void PixelArt::end_event() {
  ...
  SDL_Log("history is now %lu events; most recent has %lu tile changes",
      history.size(), field.changelist.size());
        ...
        case SDL_KEYDOWN:
          ...
          switch (e.key.keysym.sym) {
            ...
            case SDLK_DOWN:
              ...
              break;
            case 'h':
              // camera.x = camera.x - camera.z;
              app.pan(-5, 0);
              break;
            case 'j':
              // camera.y = camera.y + camera.z;
              app.pan(0, 5);
              break;
            case 'k':
              // camera.y = camera.y - camera.z;
...
// TODO(liam): fill
//             need to change up click-handling
//             releasing the mouse should start the fill. then cap the event.
//
// TODO(liam): minimap
//             full-picture display with an indicator of the edit window
