...
int main(int argc, const char* argv[]) {
  ...
  */
  ...
  char buf[4];
  for (size_t i = 0; i < mod.patterns.size(); ++i) {
    std::cout << "pattern " << std::dec << i << std::endl;
    const unsigned char* pattern = reinterpret_cast<unsigned char*>(mod.patterns[i]);
    int w = 0;
    Channel* c = new Channel;
    for (int divi = 0; divi < 64; ++divi) {
      std::cout << std::setw(2) << divi << ": ";
      for (int chani = 0; chani < 8; ++chani) {
        memcpy(&buf, &pattern[w], 4);
        w += 4;
        c->sample = (buf[0] & 0xf0) | ((buf[2] & 0xf0) >> 4);
        c->param = (((buf[0] & 0xf) << 8) | (buf[1] & 0xff)) & 0xfff;
        c->effect = (((buf[2] & 0xf) << 8) | (buf[3] & 0xff)) & 0xfff;
        printPattern(c);
      }
      std::cout << std::endl;
    }
