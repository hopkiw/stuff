...
first = [146, 399, 163, 403, 170, 393, 169, 391, 166, 386, 170, 381, 170, 371,
         ...
         113, 120, 109, 122, 102, 122, 100, 121, 95,  121, 89,  115, 87,  110,
         82,  109, 84,  118, 89,  123, 93,  129, 100, 130, 108, 132, 110, 133,
         110, 136, 107, 138, 105, 140, 95,  138, 86,  141, 79,  149, 77,  155,
         81,  162, 90,  165, 97,  167, 99,  171, 109, 171, 107, 161, 111, 156,
         113, 170, 115, 185, 118, 208, 117, 223, 121, 239, 128, 251, 133, 259,
         136, 266, 139, 276, 143, 290, 148, 310, 151, 332, 155, 348, 156, 353,
         153, 366, 149, 379, 147, 394, 146, 399]
...
second = [156, 141, 165, 135, 169, 131, 176, 130, 187, 134, 191, 140, 191,
          ...
          165, 107, 170, 99,  171, 97,  167, 89,  164, 81,  162, 77,  155,
          81,  148, 87,  140, 96,  138, 105, 141, 110, 136, 111, 126, 113,
          129, 118, 117, 128, 114, 137, 115, 146, 114, 155, 115, 158, 121,
          157, 128, 156, 134, 157, 136, 156, 136]
...
def createcoords(points):
  ...
  for i in range(len(points)):
    if i % 2 == 0:
      ret.append((points[i], points[i+1]))

  return ret


# Global to hold shapes.Line objects
lines = []

color = (200, 20, 20)
for shape in (first, second,):

  coords = createcoords(shape)
  # print('got coords', coords)

  # Prime last with first entry
  last = coords[0]
  for i in coords:
    line = shapes.Line(*i, *last, width=4, color=color,
                       batch=batch)
    last = i
    lines.append(line)
  color = (20, 200, 20)
